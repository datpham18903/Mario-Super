...
public class MapManager {
	...
	public int passMission() {
		if (getMario().getX() >= map.getEndPoint().getX()
				&& !map.getEndPoint().isTouched()) {
	...
	private void checkBottomCollisions(GameEngine engine) {
		...
		if (mario.getY() + mario.getDimension().height >= map
				.getBottomBorder()) {
	...
	private void checkEnemyCollisions() {
		...
		for (Enemy enemy : enemies) {
			...
			if (enemy.getY() + enemy.getDimension().height > map
					.getBottomBorder()) {
				enemy.setFalling(false);
				enemy.setVelY(0);
	...
	private void checkPrizeCollision() {
		...
		for (Prize prize : prizes) {
			if (prize instanceof BoostItem) {
				...
				if (boost.getY() + boost.getDimension().height > map
						.getBottomBorder()) {
					boost.setFalling(false);
					boost.setVelY(0);
					boost.setY(map.getBottomBorder()
							- boost.getDimension().height);
					if (boost.getVelX() == 0)
						boost.setVelX(2);
				}
